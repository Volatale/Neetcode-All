...
function zeroOneKnapsack(n, capacity, values, weights) {
  function getValues(capacity, i) {
    ...
    if (capacity - weights[i] >= 0) {
      maxValue = getValues(capacity - weights[i], i + 1) + values[i];
    }

    //* Case 2: Exclude the current value
    maxValue = Math.max(maxValue, getValues(capacity, i + 1));
...
//* Time: O(2^n) - At each level, we have two choices
//* So the branching factor of the function is 2
//* The height of the recursion tree scales with the number of items

//* Space: O(n) - The height of the recursion tree scales with the number of items
