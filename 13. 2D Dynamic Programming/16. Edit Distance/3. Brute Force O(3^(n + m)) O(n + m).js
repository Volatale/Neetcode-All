...
function minDistance(word1, word2) {
  function getDistance(i, j) {
    if (i === word1.length) return 0; //* No more characters to consider
    if (i === word1.length && j < word2.length) return word2.length - j; //* Insert the rest of the characters
    if (j === word2.length && i < word1.length) return word1.length - i; //* Delete the rest of the characters
...
//* Time: O(3^(n + m)) - Each call leads to three more calls in the worst case
//* The height of the recursion tree scales with the size of both word1 and word2

//* Space: O(n + m) - The height of the recursion tree scales with the size of both word1 and word2
