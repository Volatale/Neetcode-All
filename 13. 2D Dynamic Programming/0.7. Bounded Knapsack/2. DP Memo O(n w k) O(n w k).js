...
function boundedKnapsack(n, w, k, value, weight) {
  function getItems(i, capacity, k, memo) {
    ...
    if (capacity >= weight[i]) {
      maxValue = getItems(i + 1, capacity - weight[i], k - 1, memo) + value[i];
    }

    //* Case 2: Exclude the current element
    maxValue = Math.max(maxValue, getItems(i + 1, capacity, k, memo));
