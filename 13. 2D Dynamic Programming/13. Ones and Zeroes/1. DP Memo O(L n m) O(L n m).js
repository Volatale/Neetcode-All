...
function findMaxForm(strs, m, n) {
  function findSubset(i, zeroes, ones) {
    ...
    return (dp[i][zeroes][ones] = largest);
  }
  ...
  const dp = new Array(strs.length)
    .fill(0)
    .map(() => new Array(m + 1).fill(0).map(() => new Array(n + 1).fill(-1)));
...
//* Time: O(L * n * m) - We are memoizing the results of each subproblem
...
//* Space: O(L * n * m) - There will be L * m * n keys in the absolute worst case
