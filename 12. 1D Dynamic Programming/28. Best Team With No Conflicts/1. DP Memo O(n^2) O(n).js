...
//*     - We only need to cache the prev index
//*     - The key is updated with the "new" max value on the way back up
...
function bestTeamScore(scores, ages) {
  function getScore(i, prev, memo) {
    ...
    if (memo.hasOwnProperty(prev)) return memo[prev];
...
//* We are only memozing based on the prev indices [0, n]
