...
function paintHouse(house) {
  ...
  //* dp[i][c] = Min cost to paint house "i" with color "j"
  const dp = new Array(n).fill(Infinity).map(() => new Array(3).fill(0));

  //* Paint the first house all 3 colors
...
//* Space: O(n) - There are n * 3 unique subproblems
//* But since this is iterative, we can just handle all 3 cases in the same iteration
