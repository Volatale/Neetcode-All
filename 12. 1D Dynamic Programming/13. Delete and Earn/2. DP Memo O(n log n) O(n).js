...
function deleteAndEarn(nums) {
  function getScore(i, memo) {
    ...
    return (memo[i] = Math.max(getScore(nextIndex) + currSum, getScore(i + 1)));
  }
...
//* Time: O(n log n) - Sorting takes O(n log n) on average
//* There are "n" unique subproblems
//* We assume that memo[i] = Max value we can get from the first "i" elements
...
//* There are also "n" unique subproblems, so "n" keys/values in the worst case
