...
//* Apply memoization (separately for each word)
//*     - Cache the indices for each word
function findAllConcatenatedWordsInADict(words) {
  function breakWord(index, word, memo) {
    ...
    for (let i = index; i < word.length; i++) {
      ...
      if (wordDict.has(substring) && breakWord(i + 1, word, memo)) {
        memo[index] = true;
        return true;
      }
    }
...
//* We are memoizing the results of each subproblem so we lose the exponential time complexity
