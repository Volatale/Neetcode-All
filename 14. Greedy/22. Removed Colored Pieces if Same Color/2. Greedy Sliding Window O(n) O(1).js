...
//* Reset the window the moment we find a character that does NOT match the start of the window
...
function winnerOfGame(colors) {
  ...
  //* Tracks start and end of consecutive characters
  let start = 0;
  let end = 0;
  ...
  while (end < colors.length) {
    //* Colors don't match so we can't extend the window
    if (colors[start] !== colors[end]) {
      start = end;
    }

    //* Found 3 consecutive characters so we can delete one
    if (end - start + 1 >= 3) {
      colors[start] === "A" ? aliceScore++ : bobScore++;
    }
