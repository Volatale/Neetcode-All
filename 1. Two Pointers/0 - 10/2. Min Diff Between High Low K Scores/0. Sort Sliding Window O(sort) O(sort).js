...
function minimumDifference(nums, k) {
  ...
  //* We want a window of size "k" (arrays are 0-indexed)
  let start = 0;
  let end = k - 1;
...
//* Time: O(merge) - Sorting the array takes O(n log n) on average (merge sort, quick sort etc.)
//* However, it takes O(n) to find the minimum difference in the worst case

//* Space: O(merge) - The memory usage scales with the sorting algorithm used under the hood
