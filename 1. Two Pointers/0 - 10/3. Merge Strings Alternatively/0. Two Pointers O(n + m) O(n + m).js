...
//! Instead of appending to a string, we can use a Java-style string builder approach
//* Simply push each character to an array, and join the array to get the final string at the very end
function mergeAlternatively(word1, word2) {
  ...
  while (left < word1.length && right < word2.length) {
    if (left < word1.length) string.push(word1[left++]);
    if (right < word2.length) string.push(word2[right++]);
  }

  //* Handle the remaining characters in either string
  while (left < word1.length) string.push(word1[left++]);
  while (right < word2.length) string.push(word2[right++]);

  //* This is more performant than repeated string concatenations
  return string.join("");
}
