...
function isSubtree(root, subRoot) {
  ...
  const s = preorderTraversal(root, []).join("");
  const t = preorderTraversal(subRoot, []).join("");
...
function preorderTraversal(curr, results) {
  if (curr === null) {
    results.push("#"); //* Ensure we consider NULL nodes too
    return results;
  }

  //* Use separators -> Node of 12 turns into string of "12"
  //* If we compare ",12##" vs ",1##", that returns false as intended
  results.push("," + curr.val);
  preorderTraversal(curr.left, results);
  preorderTraversal(curr.right, results);
