...
function validateTree(n, leftChild, rightChild) {
  ...
  function dfs(i) {
    if (i === -1) return true; //* Null Node
    if (visited.has(i)) return false; //* The tree has a cycle

    visited.add(i); //* Mark node as visited

    //* Both the left and right children must return true
    return dfs(leftChild[i]) && dfs(rightChild[i]);
  }

  //* visited.size === n is false if there are multiple connected components
