...
function isEvenOddTree(root) {
  ...
  while (!queue.isEmpty()) {
    //* Snapshot to enable Level Order Traversal
    let size = queue.size;
    ...
    for (let i = 0; i < size; i++) {
      ...
      if (parity === 0) {
        //* Element should be ODD and GREATER THAN previous
        if (curr.val & 1 && curr.val > prevVal) {
          prevVal = curr.val;
        } else {
          return false;
        }
      } else {
        //* Element shoud be EVEN and LESS THAN previous
        if (!(curr.val & 1) && curr.val < prevVal) {
          prevVal = curr.val;
        } else {
          return false;
